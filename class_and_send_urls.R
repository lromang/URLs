...
###################################
urls_code <- read.table("urlstatus.csv",
                       sep = "\t",
...
send_ticket <- function(dep, urls, recs, dates, stat, admi, idad){
    ...
    cl_curl <- paste0('"}, "status": "new", "type": "task", "priority": "normal", "tags": ["ligas_rotas"], "requester": {"id":', idad,',"name":','"',admi,'", "email":', '"', email,'"}}', "}'",' -H "Content-Type: application/json" -v -u luis.roangarci@gmail.com:Ikidefenix131090 -X POST')
...
email     <- "luis.roangarci@gmail.com"
