...
change_lable <- function(lable){
    class <- "no disponible"
    if( !str_detect(lable, "404")
        ##str_detect(lable, "20[0-3]") |
...
###################################
missing_data <- all_data[status == "no disponible",]
...
send_ticket <- function(dep, urls, recs, dates, admi, idad, subid, email){
    ...
    cl_curl <- paste0('"}, "status": "solved", "type": "task", "priority": "normal", "tags": ["ligas_rotas"], "collaborator_ids":[',subid ,'] ,"requester": {"id":', idad,',"name":','"', admi,'", "email":', '"', email,'"}}', "}'",' -H "Content-Type: application/json" -v -u email:password -X POST')
...
#######
send_multi_ticket(missing_data)
